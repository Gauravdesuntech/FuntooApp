                ...
                <ScrollView horizontal={true} >
                    {
                        this.state.schedule.map((value, index)=>{
                            ...
                            return(
                                <TouchableOpacity key={index} onPress={() => this.onPress(value.text)} style={[styles.selectWrapper, { borderColor: color }]}>
                                    <Image source={time} style={styles.img} />
                                    <Text style={{color:Colors.textColor , textAlign:'center'}}>{value.text}</Text>
                                </TouchableOpacity>
                            )
                        })
                    }
