...
import { ListUsers, changeAssignedTask, todoList, subcat, subCategoryList, usersTaskListWithSubcat, getAllSubcatList } from '../../utils/api';
...
class NewAssignScreen extends React.Component {
    ...
    assign = () => {
        const { assignTypeID, date, enddate, selectedUsers, data, assignFrom_id } = this.state;
    ...
    setChooseTask = (v) => {
        if (this.state.assignFrom_id == '') {
            this.setState({ chooseTask_id: '', initial: -1 }, () => { alert("Please select From Person..") })
        } else {
    ...
    setTasks = (v) => {    
        ...
        const index2 = this.state.tasks.findIndex(
            item => v.id === item.id
        )
    ...
    setSections = (v) => {
        ...
        // const index2 = this.state.subCat.findIndex(
        //     item => v.id === item.id
        // )
            ...
            <>
                ...
                >
                    <TouchableOpacity onPress={() => this.setTasks(item.item)}>
                        <View style={item.item.isSelect ? styles.capsulePress : styles.capsule}>
                            <View style={{ flexDirection: "row" }}>
                                <Image source={item.item.priority} style={{ height: 30, width: 30, resizeMode: 'contain' }} />
                                <Text style={item.item.isSelect ? styles.taskTextPress : styles.taskText}>{item.item.name}{" "}
                                </Text>
                            </View>
                            <Text style={[item.item.isSelect ? styles.taskTextPress : styles.taskText, { fontSize: 12, opacity: 0.6, textAlign: "right" }]}>{item.item.date}</Text>
                        </View>
                    </TouchableOpacity>
                </ScrollView>
    ...
    renderItem = ( item ) => {
        ...
        <View style={styles.containerCart}>
             <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                {/* change icon size for major icon here */}
                <Image source={item.item.priority} style={{ height: 45, width: 45, resizeMode: 'contain' }} />
                <View style={{ paddingLeft: 15 }}>
                    <Text style={{ paddingBottom: 2, fontSize: 16, fontWeight: 'bold', color: '#7f7f7f' }}>{item.item.name}</Text>
                    <Text style={{ paddingBottom: 2, fontSize: 13, opacity: 0.6 }}>{item.item.date}</Text>
                    <Text style={{ fontStyle: 'italic', fontSize: 12, opacity: 0.5 }}>{item.item.members}</Text>
                </View>
            </View>
        <View style={{flexDirection:"row",alignItems:'center'}}>
            <View style={{ alignItems: 'center', paddingRight: 10 }}>
                {/* change icon size in image style */}
                <Image source={item.item.taskType} style={{  height: 25, width: 25, resizeMode: 'contain' }} />
                <Text style={{ fontSize: 12, opacity: 0.5 }}>{item.item.coins} Coins</Text>
            </View>
            <TouchableOpacity onPress={()=>this.deleteTask(item.item)}>
            <View>
            <Ionicons
                        name="trash-outline"
    ...
    render() {
        ...
        const { title, id } = '';
        const { date, enddate, mode, isDateTimePickerVisible, minDate } = this.state;
                            ...
                            <View style={styles.wrapper}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', width: '100%' }}>
                                    <View style={{ alignItems: 'center' }}>
                                        ...
                                        <TouchableOpacity onPress={() => { this.showDatePicker('date', 'startDate') }} style={styles.dateContainer}>
                                            <Text style={{ color: 'green' }}>{moment(date).format("DD-MM-YYYY")}</Text>
                                        </TouchableOpacity>
                                    </View>
                                    <View style={{ alignItems: 'center' }}>
                                        <Text style={{ opacity: 0.5 }}>ENDS ON</Text>
                                        <TouchableOpacity onPress={() => { this.showDatePicker('date', 'endDate') }} style={styles.dateContainer}>
                                            <Text style={{ color: 'red' }}>{moment(enddate).format("DD-MM-YYYY")}</Text>
                                        </TouchableOpacity>
                                    </View>
                    ...
                    /> */}
                            <View style={styles.modalContainer}>
                                <ScrollView nestedScrollEnabled={true} showsVerticalScrollIndicator={false}>
                                    ...
                                    <View style={[styles.selectors,{flexDirection:'row',flexWrap:'wrap'}]}>
                                        {this.state.subCat.map((selector, index) => (
                                            ...
                                            >
                                                <View style={this.state.activeSections.id == selector.id ?
                                                    styles.capsulePress : styles.capsule}>
                                                    <Text
                                                        style={
                                                            this.state.activeSections.id == selector.id ?
                                                                styles.capsuleTextPress : styles.capsuleText
                                                        }
                                                    >
                                                        {selector.name}
                                                    </Text>
