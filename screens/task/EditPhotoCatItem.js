...
import Assign from "../../components/tasks/AddTodo/Assign";
...
class EditPhotoCatItem extends React.Component {
            ...
            editTask(task_id)
                .then((response)=>{
                    ...
                    let documents = data.documents && data.documents.length > 0 ? data.documents.map((a)=>{
                        return{file:Config.DOCUMENT_URL + a,name:a,update:true} }) : null;
                    let images = data.photos && data.photos.length > 0 ? data.photos.map((a)=>{
                        return{image:Config.IMAGE_URL + a,update:true} }) : null;
                    this.setState({
                        editable:true,
                        name:data.name,
    ...
    handleSubmit=()=>{
        ...
        this.setState({
            ...
            let images = this.state.images && this.state.images.length > 0 ? this.state.images.filter(a=>{ if (!a.update){return a} }) : [];
            let document = this.state.document && this.state.document.length > 0 ? this.state.document.filter(a=>{ if (!a.update){return a} }) : [];
            if (images.length>0){
                obj.images = images
            }
            ...
            task.then((response)=>{
                    ...
                    this.props.navigation.push('CategoryItems',{id:category_id?category_id:this.state.category_id})
                }).catch(error=> {
                    this.setState({
                        loading:false
                    })
